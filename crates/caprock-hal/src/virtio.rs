//! **virtio-pci: das Auffinden der Strukturen** — die Treiberlogik liegt in `caprock-virtio`.
//!
...
//! `caprock_virtio::probe_ecam` und arbeitet auf **einer** Seite. Der alte Einwand
...
pub fn probe_transport(dev: &PciDevice) -> Option<Transport> {
    ...
    // **Derselbe** Lauf, den auch die Treiber-PD fährt (`caprock_virtio::probe_ecam`) — nicht
    ...
    //
    // Die Barriere wird HEREINGEREICHT (s. Crate-Doku von `caprock-virtio`): auf aarch64 ist
    ...
    unsafe { caprock_virtio::probe_ecam(page, cpu::dsb_sy) }
}
