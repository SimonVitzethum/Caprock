...
//! auditierte Syscall-ABI-Schicht `libsel4lake` (Allowlist). Genau das bezeugt das Zertifikat. Der
...
// SDK-Schicht libsel4lake (Allowlist) erzeugt -- dieses Programm bleibt vollstaendig forbid-sauber.
libsel4lake::entry!(run);

/// Programmlogik: die vom Loader endowte Notification (Slot 0) signalisieren, dann beenden.
fn run(_arg: usize) -> ! {
    libsel4lake::signal(NTFN_SLOT, SVC_BADGE);
    libsel4lake::exit();
}
