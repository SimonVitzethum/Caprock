#!/usr/bin/env bash
# Baut den SEL4Lake-Kernel fuer x86_64 (Branch arch/x86_64): eingebauter Bare-Metal-Target
# x86_64-unknown-none, Multiboot1-Image (Linker kernel/x86_64-link.ld). Ergebnis:
#   build/target/x86_64-unknown-none/release/sel4lake-kernel
set -euo pipefail
cd "$(dirname "$0")"
rustup run nightly cargo build --release --target x86_64-unknown-none -p sel4lake-kernel "$@"
ELF=build/target/x86_64-unknown-none/release/sel4lake-kernel
