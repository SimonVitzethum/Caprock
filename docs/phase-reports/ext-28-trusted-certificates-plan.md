...
## C0 — `caprock-trust` (Zertifikat-Parser + Ed25519-Verify) + Key-DB-Typen · host-getestet
- `crates/caprock-loader`: `cert.rs` — `TrustedCert`-Parser (fester 160-B-Layout, bounds-geprüft,
  `#![forbid(unsafe_code)]`, panik-frei); `Program.cert:&[u8]`; Archiv-Format v2 (`cert_off/len` in
  `reserved`). Host-Unit-Tests + Negativfälle.
- `crates/caprock-trust` (neu, no_std): dep `ed25519-dalek` v2 (`default-features=false`) + `sha2`.
...
- `tools/sign-trusted.py` (bzw. Rust-xtask): (1) `cargo geiger` über den App-Dep-Baum → Programm +
  projektinterne Crates **0** unsafe, `unsafe` nur in Allowlist `{libcaprock}`; sonst Abbruch +
...
## Kritische Dateien
- `crates/caprock-loader/src/{lib.rs,archive.rs,cert.rs}` — Program.cert, Archiv v2, Cert-Parser.
- `crates/caprock-trust/` (neu) — Ed25519-Verify + sha256 + fingerprint + Key-Typen.
...
- Krypto-Dep nur kernel-seitig; Programm-Trust-Basis bleibt unsafe-frei (bis auf `libcaprock`).
