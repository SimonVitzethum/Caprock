...
//! liegen im Kernel-Glue (`loader::verify_image`) bzw. in `caprock-trust`; der Kernel hält die
//! read-only Key-DB.
//!
...
/// Dependency-Allowlist erfüllt (`unsafe` nur in der erlaubten ABI-Crate, z. B. `libcaprock`).
...
/// [`Self::signature_algorithm_id`]) + die Policy erfolgen **außerhalb** (Kernel/`caprock-trust`).
